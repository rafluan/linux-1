...
{                                                                       \
        u8 read_value;                                                  \
        ov5640_read_reg(reg,&read_value);                               \
        ov5640_write_reg(reg,((read_value & (~mask))| value));          \
}
...
static int pwn1_gpio, rst1_gpio;
static int pwn2_gpio, rst2_gpio;
static struct i2c_client *bcast;
...
{
	printk("ov5640_dual_mipi: ov5640_standby (%d)\n", (int)enable);

	if (enable){
		gpio_set_value(pwn1_gpio, 1);
		gpio_set_value(pwn2_gpio, 1);
	}
	else{
		gpio_set_value(pwn1_gpio, 0);
		gpio_set_value(pwn2_gpio, 0);
	}
...
{
	printk("ov5640_dual_mipi: ov5640_reset\n");

	/* camera reset */
	gpio_set_value(rst1_gpio, 1);
	gpio_set_value(rst2_gpio, 1);
	...
	gpio_set_value(rst1_gpio, 0);
	gpio_set_value(rst2_gpio, 0);
	msleep(1);
...
static s32 ov5640_write_reg_cam1(u16 reg, u8 val)		// write only to camera 1
{
	u8 au8Buf[3] = {0};
...
static s32 ov5640_write_reg(u16 reg, u8 val)		// writes to both cameras by using the broadcast I2C address
{
	...
	if (i2c_master_send(bcast, au8Buf, 3) < 0) {
		pr_err("%s:write reg error:reg=%x,val=%x\n",
			__func__, reg, val);
		return -1;
	}
...
static void ov5640_strobe_led1_mode_mult_pulse(void)
{
	u8 reg_value;
	/*
	 * Reg 0x3B00 bit[7] - Strobe Request on/off. 0 - off, 1 - on
	 * Reg 0x3B00 bit[1:0] - Strobe mode. 00 - XENON, 01 - LED1, 10 - LED2, 11 - LED3
	 */

	// Set STROBE as an output
	printk(" Set STROBE as an output \n");
	ov5640_read_reg(0x3016,&reg_value);
	printk("%s(): %d: Reg 0x%04X Value 0x%02X \n",__func__,__LINE__,0x3016,reg_value);
	reg_value |= (1<<1);
	ov5640_write_reg(0x3016,reg_value);
	ov5640_read_reg(0x3B00,&reg_value);
	printk("%s(): %d: Reg 0x%04X Value 0x%02X \n",__func__,__LINE__,0x3016,reg_value);

	// Enable FREX Mode (FIXME: without this STROBE is not working and also frame is not coming from sensor to uP)
	printk(" Enable FREX Mode (FIXME: without this STROBE is not working and also frame is not coming from sensor to uP) \n");
	ov5640_read_reg(0x302C,&reg_value);
	printk("%s(): %d: Reg 0x%04X Value 0x%02X \n",__func__,__LINE__,0x302C,reg_value);
	reg_value |= (1<<1);
	ov5640_write_reg(0x302C,reg_value);
	ov5640_read_reg(0x3B00,&reg_value);
	printk("%s(): %d: Reg 0x%04X Value 0x%02X \n",__func__,__LINE__,0x302C,reg_value);

	// Enable Rolling Shutter Mode
	printk(" Enable Rolling Shutter Mode \n");
	ov5640_read_reg(0x3B07,&reg_value);
	printk("%s(): %d: Reg 0x%04X Value 0x%02X \n",__func__,__LINE__,0x3B07,reg_value);
	reg_value &= ~(0x3);
	reg_value |= (1<<1);
	ov5640_write_reg(0x3B07,reg_value);
	ov5640_read_reg(0x3B00,&reg_value);
	printk("%s(): %d: Reg 0x%04X Value 0x%02X \n",__func__,__LINE__,0x3B07,reg_value);

	// Enable Strobe LED1 mode
	printk(" Enable Strobe LED1 mode \n");
	ov5640_read_reg(0x3B00,&reg_value);
	printk("%s(): %d: Reg 0x%04X Value 0x%02X \n",__func__,__LINE__,0x3B00,reg_value);
	reg_value &= ~(0x83);
	reg_value |= (0x01);
	ov5640_write_reg(0x3B00,reg_value);
	ov5640_read_reg(0x3B00,&reg_value);
	printk("%s(): %d: Reg 0x%04X Value 0x%02X \n",__func__,__LINE__,0x3B00,reg_value);

	// Zero number of skipped frames
	printk(" Zero number of skipped frames \n");
	ov5640_read_reg(0x3A1C,&reg_value);
	printk("%s(): %d: Reg 0x%04X Value 0x%02X \n",__func__,__LINE__,0x3A1C,reg_value);
	reg_value = (0x00);
	ov5640_write_reg(0x3A1C,reg_value);
	ov5640_read_reg(0x3A1C,&reg_value);
	printk("%s(): %d: Reg 0x%04X Value 0x%02X \n",__func__,__LINE__,0x3A1C,reg_value);
	ov5640_read_reg(0x3A1D,&reg_value);
	printk("%s(): %d: Reg 0x%04X Value 0x%02X \n",__func__,__LINE__,0x3A1D,reg_value);
	reg_value = (0x00);
	ov5640_write_reg(0x3A1D,reg_value);
	ov5640_read_reg(0x3A1D,&reg_value);
	printk("%s(): %d: Reg 0x%04X Value 0x%02X \n",__func__,__LINE__,0x3A1D,reg_value);

	// Start Strobe Request
	printk(" Start Strobe Request \n");
	ov5640_read_reg(0x3B00,&reg_value);
	printk("%s(): %d: Reg 0x%04X Value 0x%02X \n",__func__,__LINE__,0x3B00,reg_value);
	reg_value |= (0x80);
	ov5640_write_reg(0x3B00,reg_value);
...
static void OV5640_exposure_gain_control(int enable, int gain_ctrl)
{
	u8 ex_ga_ctrl;

	ov5640_read_reg(0x3503, &ex_ga_ctrl);
	if (enable) {
		if(gain_ctrl)
			ex_ga_ctrl = ex_ga_ctrl & ~(0x02);
		else
			ex_ga_ctrl = ex_ga_ctrl & ~(0x01);
	} else {
		if(gain_ctrl)
			ex_ga_ctrl = ex_ga_ctrl | 0x02;
		else
	...
	ov5640_write_reg(0x3503, ex_ga_ctrl);
}
...
static void ov5640_set_virtual_channel(int channel)
{
	...
	ov5640_write_reg_cam1(0x4814, channel_id | (channel << 6));
}
...
static int ov5640_download_firmware(struct reg_value *pModeSetting, s32 ArySize)
{
	...
	printk("ov5640_dual_mipi: ov5640_download_firmware\n");
	...
	// set only camera 1 for gated clock mode until Bill can modify the parallel camera hw to also use this mode
	ov5640_write_reg_cam1(0x4800, 0x24);
				...
				enum ov5640_mode mode)
{
	...
	/* turn off AE/AG */
	printk("ov5640_dual_mipi: ov5640_change_mode_exposure_calc OV5640_turn_on_AE_AG(0)\n");
			    ...
			    enum ov5640_mode mode, enum ov5640_mode orig_mode)
{
	...
	else if (ov5640_data.pix.pixelformat == V4L2_PIX_FMT_GREY)
	...
	/* econsys: Virtual channel is retrieved from device tree */
	ov5640_set_virtual_channel(ov5640_data.vc);
	...
	if (mipi_csi2_info) {
		...
		printk("MIPI_CSI_PHY_STATE = 0x%x\n", mipi_reg);
...

	{
		...
		ov5640_read_reg(0x4300, &format);
		printk(KERN_ERR "%s() in %s: *4300 = 0x%x\n", __func__, __FILE__, (int)format);
		ov5640_read_reg(0x3B07, &format);
		printk(KERN_ERR "%s() LINE %d : in %s: *3B07 = 0x%x\n", __func__, __LINE__,__FILE__, (int)format);
		printk("ov5640_dual_mipi: Configure ov5640 mipi sensor for LED1 strobe mode multiple pulse output \n");
		ov5640_strobe_led1_mode_mult_pulse();
	}
...
static int ioctl_s_power(struct v4l2_int_device *s, int on)
{
	...
	u8 reg_value=0;
	printk("ov5640_dual_mipi: ioctl_s_power\n");
	...
	if (on && !sensor->on) {
		...
		if(ov5640_data.pix.pixelformat == V4L2_PIX_FMT_GREY)
		{
			if(ov5640_data.pix.width == 1280 || ov5640_data.pix.width == 1600 || ov5640_data.pix.width == 1920 || ov5640_data.pix.width == 2048 || ov5640_data.pix.width == 2592 )
			{
				ov5640_read_reg(0x3035, &reg_value);
				if(reg_value ==0x22 || reg_value ==0x12 || reg_value ==0x42)
				{
					reg_value=reg_value-1;
					ov5640_write_reg(0x3035, reg_value);
				}
			}
...
static int ioctl_s_parm(struct v4l2_int_device *s, struct v4l2_streamparm *a)
{
	...
	printk("ov5640_dual_mipi: ioctl_s_parm\n");
...
static int ioctl_s_fmt_cap(struct v4l2_int_device *s, struct v4l2_format *f)
{
	...
	case V4L2_PIX_FMT_GREY:
		...
		{
			ov5640_read_reg(0x3035, &reg_value);
			if(reg_value ==0x21 || reg_value ==0x11 || reg_value ==0x41)
			{
				reg_value=reg_value+1;
				ov5640_write_reg(0x3035, reg_value);
			}
		}
		ov5640_write_reg(0x4300, 0x10);
		ov5640_data.pix.pixelformat = format;
		break;
	default:
...
static int planckian_locus_lookuptable(int value)
{
	if (value == 1) {
/*
 * Perform Advance auto white balance
 */
		ov5640_write_reg(0x3406, 0x00);
		ov5640_write_reg(0x5181, 0x50);
		ov5640_write_reg(0x5182, 0x11);
		ov5640_write_reg(0x5183, 0x00);
		ov5640_write_reg(0x5184, 0x20);
		ov5640_write_reg(0x5186, 0x1C);
		ov5640_write_reg(0x5187, 0x18);
		ov5640_write_reg(0x5188, 0x18);
		ov5640_write_reg(0x5189, 0x56);
		ov5640_write_reg(0x518a, 0x5C);
		ov5640_write_reg(0x518B, 0xD0);
		ov5640_write_reg(0x518C, 0xBD);
		ov5640_write_reg(0x518D, 0x26);
		ov5640_write_reg(0x518F, 0x42);
		ov5640_write_reg(0x518E, 0x2B);
		ov5640_write_reg(0x5190, 0x42);
		ov5640_write_reg(0x5191, 0xF8);
		ov5640_write_reg(0x5192, 0x04);
	}else if (value == 0) {
/*
 * Perform Simple auto white balance
 */
		ov5640_write_reg(0x3406, 0x00);
		ov5640_write_reg(0x5183, 0x80);
		...
		ov5640_write_reg(0x3401, 0x13);
		ov5640_write_reg(0x3402, 0x04);
		ov5640_write_reg(0x3403, 0x00);
		ov5640_write_reg(0x3404, 0x07);
		ov5640_write_reg(0x3405, 0xE2);
	}else if (value == 5000) {
		ov5640_write_reg(0x3406, 0x01);
		...
		ov5640_write_reg(0x3402, 0x04);
		ov5640_write_reg(0x3403, 0x00);
		ov5640_write_reg(0x3404, 0x05);
		ov5640_write_reg(0x3405, 0x00);
	}else if (value == 6000) {
		ov5640_write_reg(0x3406, 0x1);
		ov5640_write_reg(0x3400, 0x7);
		ov5640_write_reg(0x3401, 0x32);
		ov5640_write_reg(0x3402, 0x4);
		ov5640_write_reg(0x3403, 0x0);
		ov5640_write_reg(0x3404, 0x5);
		ov5640_write_reg(0x3405, 0x36);
	}
	return 0;
}
...
static int ioctl_queryctrl(struct v4l2_int_device *s, struct v4l2_queryctrl *qc)
{
	...
	case V4L2_CID_COLORFX:
		return v4l2_ctrl_query_fill(qc, V4L2_COLORFX_NONE, V4L2_COLORFX_GRASS_GREEN, 1, V4L2_COLORFX_NONE);
	...
	case V4L2_CID_EXPOSURE_ABSOLUTE:
		return v4l2_ctrl_query_fill(qc, 0, 0xFFFFF, 1, 0x2A00);
	case V4L2_CID_AUTOGAIN:
		return v4l2_ctrl_query_fill(qc, 0, 1, 1, 1);
	case V4L2_CID_GAIN:
		return v4l2_ctrl_query_fill(qc, 0, 0x3FF, 1, 0x10);
	}
	return -EINVAL;
}
...
static int ioctl_s_ctrl(struct v4l2_int_device *s, struct v4l2_control *vc)
{
	int retval = 0;
	u8 reg_value = 0;
	u32 exposure_value = 0;
	unsigned char status,i;
	pr_debug("In ov5640_dual_mipi:ioctl_s_ctrl %d\n",
		 vc->id);
	...
	case V4L2_CID_BRIGHTNESS:
	{
		if ((vc->value >= -64) && (vc->value <= 64)) {
			...
			if(vc->value >= 0) {
				ov5640_write_reg(0x5587, 0xFF & (vc->value));
				WRITE_REG_WITH_OVT_MAST(0x5588, 0x00, 0x08);
			}else {
				ov5640_write_reg(0x5587, 0xFF & (-vc->value));
				WRITE_REG_WITH_OVT_MAST(0x5588, 0x08, 0x08);
			}
	...
	case V4L2_CID_CONTRAST:
	{
		if ((vc->value >= -12) && (vc->value <= 12)) {
			WRITE_REG_WITH_OVT_MAST(0x5001, 0x80, 0x80);
			WRITE_REG_WITH_OVT_MAST(0x5580, 0x04, 0x04);
			ov5640_write_reg(0x5585, 0x20 + (vc->value));
			ov5640_write_reg(0x5586, 0x20 + (vc->value));
	...
	case V4L2_CID_SATURATION:
	{
		if ((vc->value >= -48) && (vc->value <= 48)) {
			...
			ov5640_write_reg(0x5583, 0x30 + (vc->value));
			ov5640_write_reg(0x5584, 0x30 + (vc->value));
	...
	case V4L2_CID_HUE:
	{
		...
		{
			WRITE_REG_WITH_OVT_MAST(0x5001, 0x80, 0x80);
			WRITE_REG_WITH_OVT_MAST(0x5580, 0x01, 0x01);
			...
			switch(vc->value)
			{
				case -180:
				{
					...
					WRITE_REG_WITH_OVT_MAST(0x5588, 0x32, 0x33);
				}break;
				...
				case -90:
				{
					ov5640_write_reg(0x5581, 0x0);
					ov5640_write_reg(0x5582, 0x80);
					WRITE_REG_WITH_OVT_MAST(0x5588, 0x02, 0x33);
				}break;
				...
				case 0:
				{
					ov5640_write_reg(0x5581, 0x80);
					ov5640_write_reg(0x5582, 0x00);
					WRITE_REG_WITH_OVT_MAST(0x5588, 0x01, 0x33);
				}break;
				...
				case 90:
				{
					...
					ov5640_write_reg(0x5582, 0x80);
					WRITE_REG_WITH_OVT_MAST(0x5588, 0x31, 0x33);
				}break;
	...
	case V4L2_CID_VFLIP: {
		if ((vc->value >= 0) && (vc->value <= 1) ) {
			...
			ov5640_write_reg(0x3820, reg_value);
	...
	case V4L2_CID_AUTO_FOCUS_START: /* Continuous AF Start*/
		...
		ov5640_write_reg(0x3022, 0x04);	    //Enable Continuous Focus
		for(i=0 ;i<5;i++)
		{
			ov5640_read_reg(0x3023, &status);//Get Af Status
			if(status == 0x00)
			{
				break;
			}
	...
	case V4L2_CID_AUTO_FOCUS_STOP: /* Continuous AF Stop*/
		...
		ov5640_write_reg(0x3022, 0x06);	    //Release Continuous Focus
		mdelay(10);
		for(i=0 ;i<5;i++)
		{
			ov5640_read_reg(0x3023, &status);//Get Af Status
	...
	case V4L2_CID_FOCUS_ABSOLUTE:
		...
		if ((vc->value >= 0) && (vc->value <= 255)) {
			...
			ov5640_write_reg(0x3028, vc->value);
			ov5640_write_reg(0x3023, 0x01);
			ov5640_write_reg(0x3022, 0x1a);
	...
	case V4L2_CID_TEST_PATTERN: {
		...
		switch (vc->value) {
			...
			break;
			case TEST_PATTERN_COLOR_SQUARE:
				ov5640_write_reg(0x4741, 0x00);
				ov5640_write_reg(0x503d, 0x82);
			break;
			case TEST_PATTERN_BLACK_WHITE_SQUARE:
				ov5640_write_reg(0x4741, 0x00);
				ov5640_write_reg(0x503d, 0x92);
			break;
	...
	case V4L2_CID_COLORFX: {
		switch (vc->value) {
			case V4L2_COLORFX_NONE:
				...
				WRITE_REG_WITH_OVT_MAST(0x5580, 0x00, 0xF8);
			break;
			case V4L2_COLORFX_BW:
				WRITE_REG_WITH_OVT_MAST(0x5588, 0x00, 0x40);
				WRITE_REG_WITH_OVT_MAST(0x5001, 0x80, 0x80);
				WRITE_REG_WITH_OVT_MAST(0x5580, 0x18, 0x5A);
			...
			case V4L2_COLORFX_NEGATIVE:
				WRITE_REG_WITH_OVT_MAST(0x5588, 0x40, 0x40);
				WRITE_REG_WITH_OVT_MAST(0x5001, 0x80, 0x80);
				WRITE_REG_WITH_OVT_MAST(0x5580, 0x40, 0x40);
			break;
	...
	case V4L2_CID_GET_REGISTER: {
		retval = copy_from_user(&ov5640_data.reg_info,(void __user *)vc->value,sizeof(register_info));
		if (retval)
			return retval;
		ov5640_read_reg(ov5640_data.reg_info.reg,&ov5640_data.reg_info.val);
		retval = copy_to_user((void __user *)vc->value,&ov5640_data.reg_info,sizeof(register_info));
		if (retval)
			return retval;
	}break;
	...
	case V4L2_CID_SET_REGISTER: {
		...
		if (retval)
			return retval;
		ov5640_write_reg(ov5640_data.reg_info.reg,ov5640_data.reg_info.val);
	}break;
	...
	case V4L2_CID_EXPOSURE_AUTO:
		if ((vc->value >= 0) && (vc->value <= 1) ) {
			OV5640_exposure_gain_control(vc->value, 0);
			ov5640_data.ae_mode = vc->value;
		}
		break;
	case V4L2_CID_EXPOSURE_ABSOLUTE:
		if ((vc->value >= 0) && (vc->value <= 0xFFFFF)) {
			OV5640_exposure_gain_control(0, 0);
			exposure_value	= vc->value;
			ov5640_write_reg(0x3500, 0x0F & (exposure_value >> 16));
			ov5640_write_reg(0x3501, 0xFF & (exposure_value >> 8));
			ov5640_write_reg(0x3502, 0xFF & (exposure_value));
	...
	case V4L2_CID_AUTOGAIN:
		if ((vc->value >= 0) && (vc->value <= 1) ) {
			OV5640_exposure_gain_control(vc->value, 1);
			ov5640_data.ag_mode = vc->value;
		}
		break;
	case V4L2_CID_GAIN:
		if ((vc->value >= 0) && (vc->value <= 0x3FF)) {
			OV5640_exposure_gain_control(0, 1);
...
static int ioctl_dev_init(struct v4l2_int_device *s)
{
	...
	printk("ov5640_dual_mipi: ioctl_dev_init\n");
			...
			const struct i2c_device_id *id)
{
	...
	if (retval) {
		dev_err(dev, "bcast missing or invalid\n");
		return retval;
	}
	printk("ov5640_dual_mipi: bcast = 0x%x", bcast_addr);
	bcast = i2c_new_dummy(client->adapter, bcast_addr);
	if(bcast == NULL)
	{
		dev_err(dev, "error opening broadcast address\n");
		return -1;
	}

	/* request power down pin */
	pwn1_gpio = of_get_named_gpio(dev->of_node, "pwn1-gpios", 0);
	if (!gpio_is_valid(pwn1_gpio)) {
		dev_warn(dev, "no sensor pwn1 pin available");
		return -EINVAL;
	}
	retval = devm_gpio_request_one(dev, pwn1_gpio, GPIOF_OUT_INIT_HIGH,
					"ov5640_mipi_pwdn");
	if (retval < 0)
		return retval;

	/* request reset pin */
	rst1_gpio = of_get_named_gpio(dev->of_node, "rst1-gpios", 0);
	if (!gpio_is_valid(rst1_gpio)) {
		dev_warn(dev, "no sensor reset pin available");
		return -EINVAL;
	}
	retval = devm_gpio_request_one(dev, rst1_gpio, GPIOF_OUT_INIT_HIGH,
					"ov5640_mipi_reset");
	if (retval < 0)
		return retval;

	/* request power down 2 pin */
	pwn2_gpio = of_get_named_gpio(dev->of_node, "pwn2-gpios", 0);
	if (!gpio_is_valid(pwn2_gpio)) {
		dev_warn(dev, "no sensor pwdn2 pin available");
		return -EINVAL;
	}
	retval = devm_gpio_request_one(dev, pwn2_gpio, GPIOF_OUT_INIT_HIGH,
					"ov5640_mipi_pwdn2");
	if (retval < 0)
		return retval;

	/* request reset 2 pin */
	rst2_gpio = of_get_named_gpio(dev->of_node, "rst2-gpios", 0);
	if (!gpio_is_valid(rst2_gpio)) {
		dev_warn(dev, "no sensor reset2 pin available");
		return -EINVAL;
	}
	retval = devm_gpio_request_one(dev, rst2_gpio, GPIOF_OUT_INIT_HIGH,
					"ov5640_mipi_reset2");
	if (retval < 0)
		return retval;
	...
	ov5640_data.pix.width = 640;
	ov5640_data.pix.height = 480;
	ov5640_data.ae_mode = 1;
	ov5640_data.exposure_step = 0x2A00;
	ov5640_data.ag_mode = 1;
	ov5640_data.gain = 0x10;
	...
	if (retval < 0 || chip_id_high != 0x56) {
		pr_warning("camera ov5640_dual_mipi is not found (hb=0x%x, should be 0x56)\n", chip_id_high);
		clk_disable_unprepare(ov5640_data.sensor_clk);
		return -ENODEV;
	}
	retval = ov5640_read_reg(OV5640_CHIP_ID_LOW_BYTE, &chip_id_low);
	if (retval < 0 || chip_id_low != 0x40) {
		pr_warning("camera ov5640_dual_mipi is not found (lb=0x%x, should be 0x40)\n", chip_id_low);
		clk_disable_unprepare(ov5640_data.sensor_clk);
		return -ENODEV;
	}
	...
	printk("%s(): %d: Register as v4l2 device \n",__func__,__LINE__);
	retval = v4l2_int_device_register(&ov5640_int_device);
	if(retval)
		printk("%s(): %d: Failed to register as v4l2 device \n",__func__,__LINE__);

	clk_disable_unprepare(ov5640_data.sensor_clk);

	pr_info("camera ov5640_dual_mipi is found\n");
...
MODULE_DESCRIPTION("OV5640 Dual MIPI Camera Driver");
MODULE_LICENSE("GPL");
MODULE_VERSION("1.0");
MODULE_ALIAS("CSI");
